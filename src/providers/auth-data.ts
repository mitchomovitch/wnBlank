...
export class AuthData{
   ...
   * @param  {string} password [User's password]
   */
  signupUser(pseudo: string,email: string, password: string,userPhoto:any=null): any {
    return this.fireAuth.createUserWithEmailAndPassword(email, password).then((newUser) => {
      ...
      if (userPhoto != null) {
        this.profilePictureRef.child(newUser.uid).child('profilePicture.png')
      .putString(userPhoto, 'base64', {contentType: 'image/png'})
        .then((savedPicture) => {
          this.userProfile.child(newUser.uid).child('photoUrl')
